...
c.combined_weekly <- c.combined_weekly %>%
  mutate(exclude_dates = week >= as.Date("2018-11-07") & week <= as.Date("2019-01-07"))

# Plot: points for all data, smoothed lines only for non-excluded dates
ggplot(c.combined_weekly, aes(x = week, y = mean_NTL, color = source, group = source)) +
  geom_point() +
  geom_smooth(
    data = filter(c.combined_weekly, !exclude_dates),
    method = "loess", se = FALSE
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Broken Trend Line of NTL Data (Excluding Nov 7– Jan 7 from LOESS)",
       x = "Date",
       y = "NTL Value") +
  scale_color_manual(values = c("blue4", "green4")) +
...
f.camp_weeks_before <- c.combined_weekly %>%
  filter(week < as.Date("2018-11-08"))
...
ggplot(f.camp_days_before, aes(x = date, y = Gap_Filled_DNB_BRDF_Corrected_NTL, color = source, group = source)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Smooth Trend Line of Before-Fire NTL Data (Comparison)",
       x = "Date",
       y = "NTL Value") +
  scale_color_manual(values = c("blue4", "green4")) 
...
f.camp_weeks_after <- c.combined_weekly %>%
  filter(week >= as.Date("2019-01-07"))
...
end_before <- max(ntl_diff_data$date[ntl_diff_data$date < as.Date("2018-11-09")])
start_after <- min(ntl_diff_data$date[ntl_diff_data$date >= as.Date("2019-01-07")])
...
f.before <- ntl_diff_data %>%
  filter(date < as.Date("2018-11-09")) %>%
...
break_adjusted <- max(f.before$date_adjusted) + 1
...
ggplot(f.all_adjusted, aes(x = date_adjusted, y = NTL_diff)) +
  ...
  annotate("text", x = break_adjusted + 2, y = max(f.all_adjusted$NTL_diff, na.rm = TRUE),
           label = "Camp Fire\nNov 9–Jan 6 excluded", color = "red", hjust = 0) +
  labs(
