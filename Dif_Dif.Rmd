...
plot(fect_model_log, type = "gap") +
  ...
  labs(y = "Estimated % Change in GDP (via NTL)",
       ...
       title = "Elasticity of GDP (via NTL) to Wildfire Exposure")
...
df_fect_clean %>%
  filter(D == 1) %>%
  group_by(id) %>%
  summarise(mean = mean(log_mean, na.rm = TRUE)) %>%
  arrange(desc(mean)) %>%
  head(10) %>%
  ggplot(aes(x = reorder(id, -mean), y = mean)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Top 10 Units with Highest Log Mean Post-Fire",
       x = "Unit ID",
       y = "Mean Log GDP (via NTL)") +
  theme_minimal()
