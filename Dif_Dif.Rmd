...
df <- read.csv("ccombined.csv")
df$date <- as.Date(df$date)

# Fire treatment window
fire_start <- as.Date("2018-11-08")
fire_end <- as.Date("2018-12-06")

# Create treatment period indicator
df$treatment_period <- ifelse(df$date >= fire_start & df$date <= fire_end, 1, 0)

# Create numeric unit IDs from source (e.g., Buffer = 1, Camp Fire = 2)
df$unit <- as.numeric(as.factor(df$source))

# Order by unit and date
df <- df[order(df$unit, df$date), ]

# Create numeric time index
df$time <- as.numeric(factor(df$date))

# Get numeric time index corresponding to fire_start
fire_time_index <- unique(df$time[df$date == fire_start])

# Create treatment cohort variable G
# Set to fire_time_index for Camp Fire group, 0 for Buffer (never treated)
# First, identify treatment group value
treatment_unit_value <- df$unit[df$source == "Camp Fire"][1]
df$G <- ifelse(df$unit == treatment_unit_value, fire_time_index, 0)

# Rename for use with att_gt()
df$luminosity <- df$Gap_Filled_DNB_BRDF_Corrected_NTL

# Run event-study-style difference-in-differences
library(did)
...
# Aggregate ATT dynamically
agg_dynamic <- aggte(att_gt_out, type = "dynamic")
...
df_fect_clean <- df_fect %>%
  ...
  filter(date < as.Date("2018-11-08") | date >= ("2019-01-07"))
...
plot(fect_model_log, type = "gap") +
  ...
  labs(y = "Estimated % Change in GDP (via NTL)",
       x = "Weeks Since Wildfire",
       title = "Estimated GDP Change Post-Wildfire Using FECT (Interactive Fixed Effects)")
...
if (!is.null(fect_model_log$att.boot.sd)) {
  se_time0 <- fect_model_log$att.boot.sd[time0_idx]
  z_stat <- abs(mean_time0 / se_time0)
  p_value_time0 <- 2 * (1 - pnorm(z_stat))
} else {
  p_value_time0 <- NA  # or manually specify
}

# Format p-value nicely
p_value_label <- if (!is.na(p_value_time0)) {
  paste0("p-value at time 0: ", signif(p_value_time0, 3))
} else {
...
plot(fect_model_log, type = "gap") +
  ...
  annotate("text", x = 0, y = 0.18, label = mean_label, hjust = 0, size = 4, color = "blue") +
  annotate("text", x = 0, y = 0.15, label = p_value_label, hjust = 0, size = 4, color = "red")
